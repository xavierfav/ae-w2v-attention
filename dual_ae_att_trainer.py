...
class DualAEAttTrainer():
    def __init__(self, params):
        ...
        self.max_num_tags = 10
        self.aggregation = params['aggregation']
        self.num_heads = params['num_heads']
        self.size_w2v = params['size_w2v']
    ...
    def init_models(self):
        ...
        self.audio_decoder = AudioDecoder()
            ...
            self.tag_encoder = TagMeanEncoder(1001, self.size_w2v, self.size_w2v)
        elif self.aggregation == 'self':
            self.tag_encoder = TagSelfAttentionEncoder(
                1001, self.size_w2v, self.num_heads, self.size_w2v, self.size_w2v, self.size_w2v
            )
    ...
    def load_model_checkpoints(self):
        ...
        try:
            ...
            self.tag_encoder.load_state_dict(torch.load(str(Path(f'saved_models', self.experiment_name, f'tag_encoder_att_epoch_{last_epoch}.pt'))))
    ...
    def train_one_epoch_dual_AE(self, epoch):
        ...
        for batch_idx, (data, tags, sound_ids) in enumerate(self.train_loader):
            ...
            tags_input = torch.tensor(tag_idxs, dtype=torch.long).to(self.device)
        ...
        if epoch%self.save_model_every == 0:
            ...
            torch.save(self.tag_encoder.state_dict(), str(Path(f'saved_models', self.experiment_name, f'tag_encoder_att_epoch_{epoch}.pt')))
